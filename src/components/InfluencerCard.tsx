...
export default function InfluencerCard({ 
  ...
  return (
    <Link 
      ...
      className={cn(
        "block relative rounded-xl overflow-hidden transition-all duration-300",
        "transform hover:translate-y-[-4px] hover:shadow-card",
    ...
    >
      <div className="relative aspect-[3/2] overflow-hidden">
        ...
        <div className="absolute top-3 right-3 z-20">
          <div className={cn(
            "flex items-center gap-1 py-1 px-2 rounded-full text-xs font-medium",
            hasPositiveChange 
              ? "bg-influence-success/90 text-white" 
              : "bg-influence-error/90 text-white"
          )}>
        ...
        <div className="absolute bottom-3 left-3 z-20">
          {influencer.categories.slice(0, 2).map((category, i) => (
            <span 
              key={i}
              className="inline-block mr-1.5 mb-1.5 py-0.5 px-2 bg-black/40 backdrop-blur-sm text-white text-xs rounded-full"
            >
              {category}
            </span>
          ))}
      ...
      <div className="p-4 relative">
        <div className="absolute right-4 top-4">
          <div className="flex items-center gap-1 text-influence-accent">
